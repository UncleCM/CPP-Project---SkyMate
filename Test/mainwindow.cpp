        ...
        , ui(new Ui::MainWindow)
    {
        ...
        // Connect MainWindow signals to Search slots
        connect(this, &MainWindow::currentWeatherDataReceived, search, &Search::currentWeatherDataReceived);
        connect(this, &MainWindow::fiveDayWeatherDataReceived, search, &Search::fiveDayWeatherDataReceived);
            ...
            if (currentWeatherReply->error() == QNetworkReply::NoError) {
                ...
                connect(forecastWeatherReply, &QNetworkReply::finished, this, [=]() {
                    if (forecastWeatherReply->error() == QNetworkReply::NoError) {
                        ...
                        updateUIFromFiveDayForecast(forecastWeatherJsonObject);
                        ...
                        // Connect the finished signal for the air quality request
                        connect(airQualityReply, &QNetworkReply::finished, this, [=]() {
                            if (airQualityReply->error() == QNetworkReply::NoError) {
                                ...
                                qDebug() << "Error fetching air quality data: " << airQualityReply->errorString();
                            }

                            // Clean up the air quality reply
                    ...
                    // Continue with Air Quality request and processing...
                });
            } else {
                qDebug() << "Error fetching current weather data: " << currentWeatherReply->errorString();
            }
    ...
    void MainWindow::updateUIFromApiResponse(const QJsonObject &jsonObject)
    {
        ...
        currentWeatherData["temperature"] = jsonObject["main"].toObject()["temp"].toDouble();
        QJsonArray weatherArray = jsonObject["weather"].toArray();
        if (!weatherArray.isEmpty()) {
            QJsonObject weatherObject = weatherArray.at(0).toObject();
            currentWeatherData["icon"] = weatherObject["icon"].toString();
        }
        emit currentWeatherDataReceived(currentWeatherData);
    }
    ...
    void MainWindow::updateUIFromFiveDayForecast(QJsonObject forecastWeatherJsonObject)
    {
        ...
        emit fiveDayWeatherDataReceived(fiveDayWeatherData);
    }
    ...
    void MainWindow::on_comboBox_currentIndexChanged(int index)
    {
        ...
        // Fetch weather data for the selected city
        currentReply = makeApiRequest(cityName);
        ...
        connect(currentReply, &QNetworkReply::finished, this, [=]() {
            if (currentReply->error() == QNetworkReply::NoError) {
                ...
                qDebug() << "Error fetching data for City " << cityName << ": " << currentReply->errorString();
                // Reset wind speed label to "N/A" in case of an error
                ui->windspeed_value->setText("N/A");
            }
